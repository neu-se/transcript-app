...
app.post("/transcripts/:studentID/:course", (req, res) => {
  try {
    ...
    if (!grade) {
      res
        .status(400)
        .send(`Invalid grade, must be a number. Got ${req.body.grade}`);
      return;
    }
    console.log(
...
app.get("/courses", (req, res) => {
  try {
    console.log("getting courses list with discription");
    const courses = db.getCourseDescriptions();
...
app.get("/courses/:course", (req, res) => {
  try {
    console.log("getting a course with name");
    const courseName = req.params.course
    const course = db.getCourseDescription(courseName)
    if (course === undefined) {
      throw new Error(`Cannot find course with this name ${courseName}`);
    } else {
      res.status(200).send(course.description)
    }
  } catch (e) {
    res.status(400).send(e);
  }
